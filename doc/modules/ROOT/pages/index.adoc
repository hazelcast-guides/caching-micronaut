...
To configure caching with Hazelcast, we'll by adding some dependencies to our `pom.xml` file:
[source,xml]
----
<dependency>
...
Our application is now ready to go. We don't need to change anything else because we're already using Micronaut caching annotations in the `CommandService` class.
...
Firstly, open another terminal and run the following command to put the data into Hazelcast distributed map:
[source,shell]
----
